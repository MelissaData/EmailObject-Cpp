<#
.SYNOPSIS
    Downloads the required components and then builds and runs MelissaEmailObjectWindowsCpp
...
.DESCRIPTION
    ...
    inside the MSVC build environment and runs it against the supplied email address.
    ...
    Overall flow:
      ...
      4. Build with nmake (via BuildProgram.ps1) and run (single test email address or interactive).
...
.EXAMPLE
    .\MelissaEmailObjectWindowsCpp.ps1 -license "your-license"

.EXAMPLE
    .\MelissaEmailObjectWindowsCpp.ps1 -email "info@melissa.com" -license "your-license"
#>
...
$ProjectPath = "$CurrentPath\MelissaEmailObjectWindowsCpp"
...
function DownloadDLLs() {
  ...
  foreach ($DLL in $DLLs) {
    ...
    if ($quiet) {
      if ($DLL.FileName -eq "mdEmail.dll")
      {
...
function CheckDLLs() {
  ...
  if (!(Test-Path ("$ProjectPath\mdEmail.h"))) {
    Write-Host "mdEmail.h not found." 
    $FileMissing = $true
  }
  if (!(Test-Path ("$ProjectPath\mdEmail.lib"))) {
    Write-Host "mdEmail.lib not found." 
    $FileMissing = $true
  }
...
Write-Host "`n======================= Melissa Email Object =======================`n                     [ C++ | Windows | 64BIT ]`n"
...
if ([string]::IsNullOrEmpty($email)) {
  & $BuildPath\MelissaEmailObjectWindowsCpp.exe --license $License  --dataPath $DataPath
}
else {
  & $BuildPath\MelissaEmailObjectWindowsCpp.exe --license $License  --dataPath $DataPath --email $email
}
